...
# Integration keys the classic form posts BARE (no integrations_ prefix):
# anything in omlx IntegrationsSettings reached via saveIntegrationSettings.
INTEGRATION_BARE_KEYS = {
    "markitdown_enabled", "markitdown_expose_model",
    "markitdown_max_file_size_mb", "markitdown_max_files_per_request",
    "markitdown_pdf_processing_engine",
    "web_search_provider", "web_search_brave_api_key", "web_search_searxng_url",
    "web_search_ddgs_backends", "web_search_max_results",
    "web_search_content_mode", "web_search_content_truncate",
...
def sim_tick(speed):
    ...
    with LOCK:
        ...
        # Backlog guard: without a cap the queue grows unbounded because the
        # spawn rate (up to ~3/s) far exceeds service (2 concurrent prefills,
        # big prompts occupy a slot for minutes). Keep the sim realistic:
        # spawn only while the pending backlog is small.
        pending = sum(1 for x in REQUESTS.values() if x["origin"] == "sim"
                      and x["state"] in ("queued", "prefilling", "generating"))
        if loaded and pending < 8 and RNG.random() < min(0.95, 1.25 * speed):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/admin/api/stats":
            ...
            # Upstream task dicts carry no 'kind' field; treat a missing one
            # as this list's own kind, otherwise every real task is dropped.
            # Also normalise id: upstream tasks use 'task_id', the UI cancel
            # path reads 'id'.
            upstream_rows = []
            for t in (base.get("tasks") or []):
                if t.get("kind") in kinds[key] or "kind" not in t:
                    t = dict(t)
                    t.setdefault("id", t.get("task_id") or "")
                    t.setdefault("kind", key)
                    # upstream reports progress 0-100, taskRow expects 0-1
                    if isinstance(t.get("progress"), (int, float)) and t["progress"] > 1:
                        t["progress"] = t["progress"] / 100.0
                    # upstream sizes ride in total_size; taskRow reads 'size'
                    if "size" not in t and t.get("total_size"):
                        t["size"] = t["total_size"]
                    upstream_rows.append(t)
            with LOCK:
    ...
    def do_POST(self):
        ...
        if p == "/admin/api/global-settings":
            # classic saveIntegrationSettings() sends integrations_* ONLY for
            # CLI-assistant keys and BARE markitdown_*/web_search_* keys;
            ...
            for k, v in body.items():
                if k in GS_FLAT_MAP or k == "api_key" or k in INTEGRATION_BARE_KEYS:
                    changed[k] = v
            with LOCK:
